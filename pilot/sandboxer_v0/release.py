...
from .report_latex import build_report_latex
...
def build_release(bundle:Any,*,review:Mapping[str,Any],video:Mapping[str,Any],licenses:Sequence[str],known_limitations_signature:str,broadcast:Mapping[str,Any]|None=None,compile_latex:bool=False)->dict[str,Any]:
    ...
    report=build_result_report(evidence); model=report.model.to_dict()
    latex=build_report_latex(evidence,compile_pdf=compile_latex)
    report_ref={"source_bundle_hash":source,"json":report.json,"html":report.html,"pdf_sha256":hashlib.sha256(report.pdf).hexdigest(),"latex_tex":latex.tex,"latex_tex_sha256":hashlib.sha256(latex.tex.encode()).hexdigest(),"latex_pdf_sha256":hashlib.sha256(latex.pdf).hexdigest() if latex.pdf else None,"latex_charts":sorted(latex.charts.keys())}
...
def _site(model:Mapping[str,Any],source:str,replay:Mapping[str,Any],video:Mapping[str,Any],*,video_url:str|None=None)->dict[str,Any]:
    ...
    document=f'''<!doctype html><html lang="en"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Sandboxer result</title><main><h1>{html.escape(model["title"])}</h1><p>{html.escape(model["disclaimer"])}</p><nav aria-label="Series and Match navigation"><ul>{chapters}</ul></nav><p><a href="report.html">Canonical report</a> · <a href="report-detailed.pdf">Detailed report (PDF)</a> · <a href="report-detailed.tex">LaTeX source</a> · <a href="replay.json">Replay</a> · <a href="video.mp4">Video</a> · <a href="evidence.json">Evidence</a>{youtube}</p><section aria-labelledby="corrections"><h2 id="corrections">Correction history</h2><p>{corrections}</p></section></main></html>'''
