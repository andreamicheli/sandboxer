...
def build_release(bundle:Any,*,review:Mapping[str,Any],video:Mapping[str,Any],licenses:Sequence[str],known_limitations_signature:str)->dict[str,Any]:
...
def _site(model:Mapping[str,Any],source:str,replay:Mapping[str,Any],video:Mapping[str,Any])->dict[str,Any]:
    ...
    document=f'''<!doctype html><html lang="en"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Sandboxer result</title><main><h1>{html.escape(model["title"])}</h1><p>{html.escape(model["disclaimer"])}</p><nav aria-label="Series and Match navigation"><ul>{chapters}</ul></nav><p><a href="report.html">Canonical report</a> · <a href="replay.json">Replay</a> · <a href="video.mp4">Video</a> · <a href="evidence.json">Evidence</a></p><section aria-labelledby="corrections"><h2 id="corrections">Correction history</h2><p>{corrections}</p></section></main></html>'''
