...
class DashboardApp:
    ...
    def __call__(self,environ,start_response):
        ...
        if method!="GET": return self._reply(start_response,405,{"error":"METHOD_NOT_ALLOWED"},"private, no-store",remote,path)
    ...
    def _authorized(self,header:str)->bool:
        ...
        return hmac.compare_digest(self.hash_token(supplied),self.token_hash)
    ...
    def _raw(self,start,status:int,body:bytes,content_type:str,cache:str,remote:str,path:str):
        labels={200:"OK",401:"Unauthorized",404:"Not Found",405:"Method Not Allowed",429:"Too Many Requests"}
        start(f"{status} {labels[status]}",[("Content-Type",content_type),("Content-Length",str(len(body))),("Cache-Control",cache),("X-Content-Type-Options","nosniff"),("X-Frame-Options","DENY"),("Referrer-Policy","no-referrer"),("Content-Security-Policy","default-src 'none'; style-src 'unsafe-inline'")])
