...
function renderComparison(match){
  ...
  target.replaceChildren(...comparable.map(row=>{const other=leftBench.get(row.id),card=document.createElement('article'),title=document.createElement('h3');text(title,row.label);card.append(title);for(const [model,value] of [[left,other.value],[right,row.value]]){const line=document.createElement('div'),name=document.createElement('span'),score=document.createElement('b');text(name,model.public_name);text(score,String(value));line.append(name,score);card.append(line)}return card}));
}
